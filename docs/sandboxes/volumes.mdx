...
- `deny=<pattern>` hides host paths matching a gitignore-style pattern from the guest: matching entries are invisible (ENOENT) and writes to them are forbidden (EACCES). Patterns are relative to the mount root and may be names (`.env`, `*.log`) or paths (`sub/secret`). Repeatable to deny several patterns. Name and path patterns are enforced on every host platform (Linux, macOS, and Windows). Matching follows the case-sensitivity of the mount's filesystem: on a case-insensitive host filesystem (Windows NTFS, default macOS APFS) a pattern such as `.env` also hides `.ENV` and `.Env`; on a case-sensitive host (Linux ext4, a case-sensitive APFS volume) matching is exact. **Patterns are location-based, not confidentiality guarantees.** A path pattern like `sub/.env` is checked against each entry's mount-relative path at lookup time; a guest with a writable mount can `rename` an ancestor directory (for example `mv sub x`) so that the denied path no longer matches and the file is served. Use **name-only patterns** (`.env`, `*.log`) when you need a denied entry to stay hidden regardless of the directory it lives in, and prefer `ro` mounts when the guest should not be able to alter the tree at all.
...
| Flag | Source | Options |
|------|--------|---------|
| `--mount-dir` | Host directory | `ro`, `rw`, `noexec`, `nosuid`, `nodev`, `stat-virt=strict\|relaxed\|off`, `host-perms=private\|mirror`, `deny=<pattern>` (repeatable) |
