...
- `deny=<pattern>` hides host paths matching a gitignore-style pattern from the guest: matching entries are invisible (ENOENT) and writes to them are forbidden (EACCES). Patterns are relative to the mount root and may be names (`.env`, `*.log`) or paths (`sub/secret`). Repeatable to deny several patterns.
